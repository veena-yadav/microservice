...
const controller = {
  PlaceOrder: expressAsyncHandler(async (req, res) => {
    ...
    req.body.orders.forEach((element) => {
      ...
      Item.findOne(
        query,
        expressAsyncHandler(async (err, medicine) => {
          ...
          if (err) {
            ...
            if (medicine.quantity == 0) {
              ...
              if (element.quantity <= medicine.quantity) {
                ...
                await drivers.addToReorderBucket_UserDB(
                  element,
                  req.body.email
                );
                element.quantity = medicine.quantity;
              ...
              Medicine.findOne(query, async (err, result) => {
                if (err) {
                  ...
                  await newMedicine.save().then(() => res.json("not found"));
                  //   .catch((err) => res.status(400).json("Error: " + err));
                }
              });
            } else {
...
const operationsOnOrders = {
  ...
  fetchOrdersByEmailSortByPriceAsc: expressAsyncHandler(
  ...
  fetchOrdersByEmailSortByPriceDesc: expressAsyncHandler(
    async (request, response) => {
      let queryUserDb = {
        email: { $regex: request.body.email },
      };
  ...
  fetchOrdersByEmailSortByQuantityAsc: expressAsyncHandler(
    async (request, response) => {
      let queryUserDb = {
        email: { $regex: request.body.email },
      };
  ...
  fetchOrdersByEmailSortByQuantityDesc: expressAsyncHandler(
    async (request, response) => {
      let queryUserDb = {
        email: { $regex: request.body.email },
      };
  ...
  deleteAnOrder: expressAsyncHandler(async (request, response) => {
    ...
    await userDb
      .findOneAndUpdate(queryUserDb, {
        $pull: { order_bucket: { itemName: request.body.itemName } },
      })
...
const operationsOnReorders = {
  ...
  fetchReordersByEmailSortByPriceAsc: expressAsyncHandler(
    async (request, response) => {
      let queryUserDb = {
        email: { $regex: request.body.email },
      };
  ...
  fetchReordersByEmailSortByPriceDesc: expressAsyncHandler(
    async (request, response) => {
      let queryUserDb = {
        email: { $regex: request.body.email },
      };
  ...
  fetchReordersByEmailSortByQuantityAsc: expressAsyncHandler(
    async (request, response) => {
      let queryUserDb = {
        email: { $regex: request.body.email },
      };
  ...
  fetchReordersByEmailSortByQuantityDesc: expressAsyncHandler(
    async (request, response) => {
      let queryUserDb = {
        email: { $regex: request.body.email },
      };
  ...
  deleteAnReorder: expressAsyncHandler(async (request, response) => {
    ...
    await userDb
      .findOneAndUpdate(queryUserDb, {
        $pull: { reorder_bucket: { itemName: request.body.itemName } },
      })
      .then(() => response.json("deleted"))
      .catch((err) => response.json("Error : " + err));
  }),
...
const drivers = {
  addToReorderBucket_UserDB: expressAsyncHandler(async (order, email) => {
    ...
    if (reorderExist) {
      await userDb
        .updateOne(
          queryUserDb,
          { $inc: { "reorder_bucket.$.quantity": order.quantity } },
          { new: true }
        )
        .then(() => {
          console.log(
            "Successfully updated in the reorder bucket and reorder exists"
          );
        })
        .catch((err) => {
  ...
  addToOrderBucket_UserDB: expressAsyncHandler(async (order, email) => {
    ...
    if (orderExist) {
      await userDb
        .updateOne(
          queryUserDb,
          { $inc: { "order_bucket.$.quantity": order.quantity } },
          { new: true }
        )
        .then(() => {
          console.log(
            "Successfully updated in the order bucket and reorder exists"
          );
        })
        .catch((err) => {
          console.log("Error : " + err);
  ...
  updateMedicineDB: async (order) => {
    ...
    Item.findOne(queryMedicineDb, async (err, result) => {
      ...
      Item.findByIdAndUpdate(result.id, { $set: { quantity } }).then(() => {
